...
class AgentOrchestrator:
    ...
    def simulate(s, periods=365, seed=42):
        ...
        demand = np.maximum(rng.normal(1000, 200, periods), 0)
        results = []; inv = 1000*2; cost = 0
        for t in range(periods):
            d = demand[t]
            order = max(0, d*1.1 - inv + 1000)
        ...
        return {{"total_cost": round(cost,0), "avg_inventory": round(np.mean([r["inventory"] for r in results]),1),
                ...
                "periods": len(results)}}
if __name__=="__main__": print(AgentOrchestrator().simulate())
